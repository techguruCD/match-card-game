...
class ConfettiWorld {
    ...
    init(elem) {
        this.objects = [];
        elem.addEventListener('click', this.explode.bind(this));
    }
...
class Universe {
    ...
    constructor(element) {
        ...
        this.updateSize();
        // this.el.addEventListener('resize', this.updateSize.bind(this));
    ...
    addWorld(worldName, World) {
        ...
        this.worlds[worldName].init(this.el);
    }
...
function CardGame(gamePanel, options) {
    this.initHtml = () => {
            ...
            `<div class="result-panel panel">
                <div class="result-container">
                    ...
                    <div class="result-inner-content-right">
                        ...
                        <div class="result-item result-item-time">
        ...
        let toolBarHtml =
            `<div class="tool-bar">
                ...
                <div class="tool-bar-item tool-bar-item-data tool-bar-item-timer">
                    <img src="./assets/image/icons-SVG/icon-11-time.svg">
                    <span>0:37</span>
                </div>
        ...
        let totalHtml = `${captionHtml}${panelContainerHtml}<canvas> </canvas>`;
    ...
    this.initCardBoard = () => {
        ...
        $(panels.card).find(`.card-board-container .card-board-mode-${this.gameStatus.cardMode} .card-item.unmatched`).removeClass('unmatched');
        ...
        $(cardItem).click(() => {
            ...
            if (!activeCardItem) {
                ...
                if ($(cardItem).attr('data-index') == $(activeCardItem).attr('data-index')) {
                    ...
                    $(cardItem).addClass('active');
                    $(cardItem).addClass('unmatched');
                    $(activeCardItem).addClass('unmatched');
                    this.gameStatus.cardBoard.lock = true;
                    setTimeout(() => {
                        this.gameStatus.cardBoard.lock = false;
                        $(cardBoard).find('.card-item.active').removeClass('unmatched');
    ...
    const element = $(gamePanel).find('canvas')[0];
    this.confetti = new Universe(element);
    return this;
}